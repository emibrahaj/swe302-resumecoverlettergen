...
DEFAULT_RECIPIENT = "diversihire@gmail.com"
...
def _try_send_email(payload: ContactMessage, recipient: str) -> tuple[
    bool, Optional[str]]:
    ...
    msg = EmailMessage()
    msg[
        "Subject"] = f"[DiversiHire contact] {payload.subject or 'New message'}"
    msg["From"] = from_addr
    msg["To"] = recipient
...
async def send_contact_message(payload: ContactMessage):
    ...
    record = {"id": str(uuid.uuid4()), "name": payload.name,
        "email": payload.email, "subject": payload.subject or "",
        "body": payload.message, "recipient": recipient,
    ...
    try:
        ...
        raise HTTPException(status_code=500,
                            detail=f"Could not save message: {exc}")

    return {"status": "received", "delivered_via_email": delivered,
        "recipient": recipient, "message": (
            "Your message has been received." if delivered else "Your message has been received. Live email delivery is not yet configured on this server — the team will see it in the message log."), }
...
async def list_messages():
    ...
    db_client: Client = db.get_db()
    res = (
        db_client.table("contact_messages").select("*").order("created_at",
                                                              desc=True).limit(
            50).execute())
