...
export function CVBuilder({
                              templateId, resumeId: initialResumeId
                          }: CVBuilderProps) {
    ...
    const {
        resume: loadedResume, loading: loadingResume
    ...
    const updateWorkExperience = (id: string, field: keyof WorkExperience, value: string) => {
        setWorkExperience(workExperience.map((exp) => exp.id === id ? {
            ...exp,
            [field]: value
        } : exp));
    };
    ...
    const updateCustomItem = (sectionId: string, index: number, value: string) => {
        setCustomSections(customSections.map((section) => section.id === sectionId ? {
            ...section,
    ...
    const handleAiEnhance = async () => {
        ...
        try {
            ...
            const pAbout = String((polished as {
                about?: unknown; summary?: unknown
            }).about ?? (polished as {
                summary?: unknown
            }).summary ?? personalInfo.summary);
            setPersonalInfo((p) => ({
                ...p, fullName: pName, title: pTitle, summary: pAbout
            }));
            ...
            if (Array.isArray(expsAny) && expsAny.length > 0) {
                setWorkExperience(expsAny.map((e: Record<string, unknown>, i: number) => ({
            ...
            if (Array.isArray(skillsAny)) {
                const fresh: Skill[] = skillsAny.map((s: unknown, i: number) => {
                    if (typeof s === "string") return {
                        id: String(i), name: s, proficiency: "Intermediate"
                    };
    ...
    useEffect(() => {
        ...
        // keep the rest of your existing hydration code here
    }, [loadedResume]);
    ...
    const buildRawContent = () => ({
        ...
        photo_url: cvPhoto ?? "",
        skills: skills.map((s) => ({
            skill_id: s.id, skill_name: s.name, proficiency: s.proficiency
        })),
    ...
    const renderSectionWrapper = ({
        ...
        return (<div
            ...
            className={`bg-white rounded-xl border-2 transition-colors ${isOver ? "border-[#088395] shadow-md" : "border-gray-200"}`}
        >
            <div
    ...
    const renderEditorSection = (id: SectionId) => {
        if (id === "experience") {
            return renderSectionWrapper({
                ...
                >
                    ...
                    {workExperience.map((exp, expIdx) => (<div
                        key={exp.id}
                        ...
                        <label
                            ...
                            <input
                                ...
                                onChange={(e) => {
                                    ...
                                    setWorkExperience((prev) => prev.map((x) => x.id === exp.id ? {
                                        ...x,
                                        isCurrent: checked,
                ...
                id, children: (<>
                    <div className="space-y-2 mb-3">
                        {skills.map((skill) => (<div key={skill.id}
                                    ...
                                    <span
                                        className="flex-1 px-3 py-1.5 bg-[#088395]/10 text-[#088395] rounded-lg text-sm font-medium truncate">
                                        {skill.name}
                                    </span>
                            <select
                                value={skill.proficiency}
                                onChange={(e) => setSkills(skills.map((s) => s.id === skill.id ? {
                                    ...s,
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            {SKILL_PROFICIENCY.map((l) => <option
                                key={l}>{l}</option>)}
                        </select>
                        <button
                            type="button"
            ...
            return renderSectionWrapper({
                ...
                children: projects.length === 0 ? (
                    <p className="text-gray-400 text-sm">No projects
                        added yet</p>) : (<>
                    {projects.map((project, projIdx) => (<div
                        key={project.id}
                        className="space-y-3 p-4 bg-gray-50 rounded-lg mb-3 border border-gray-100"
                    >
                        <div className="flex gap-2">
        ...
        if (id === "languages") {
            ...
            return renderSectionWrapper({
                id,
                addButton: (<button type="button" onClick={addLanguage}
                                    className="flex items-center gap-1 text-xs text-[#088395] hover:text-teal-700 ml-1">
                    <Plus size={14}/> Add
                </button>),
            ...
            return renderSectionWrapper({
                ...
                children: certifications.length === 0 ? (
                    <p className="text-gray-400 text-sm">No certifications
                        added yet</p>) : (<>
                    {certifications.map((cert) => (<div key={cert.id}
                ...
                <div
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 border-2 border-gray-200 rounded-lg hover:bg-gray-50 transition-all ${saving || loadingResume ? "opacity-60 cursor-not-allowed" : ""}`}
                        >
                            {showLoginGate ? (<Lock size={14}/>) : (
                                <Save size={16}
                        ...
                        <button
                            ...
                            className={`flex items-center gap-2 px-4 py-2 bg-[#088395] text-white rounded-lg hover:shadow-lg transition-all ${downloading ? "opacity-75 cursor-wait" : ""}`}
                        >
                            {showLoginGate ? (<Lock size={14}/>) : (
                                <Download size={16}
                        ...
                        <div className="flex border-b border-gray-200">
                            {(["content", "design"] as const).map((tab) => (
                                <button
                                    ...
                                    className={`flex-1 px-6 py-4 font-semibold capitalize ${activeTab === tab ? "text-[#088395] border-b-2 border-[#088395]" : "text-foreground/70"}`}
                                >
                                    {tab}
                                </button>))}
                        </div>
                                        ...
                                        <div
                                            ...
                                            {[
                                                ...
                                                <button
                                                    ...
                                                    disabled={customSections.some((section) => section.title === cat)}
                                                    className={`px-3 py-1.5 rounded-lg border-2 text-sm transition-colors ${customSections.some((section) => section.title === cat) ? "border-gray-200 bg-gray-100 text-gray-400 cursor-not-allowed" : "border-[#088395]/30 text-[#088395] hover:bg-[#088395]/5"}`}
                                                >
                                                    {cat}
                                                </button>))}
                                        </div>
                                    ...
                                    <div
                                        ...
                                        {(["single", "two"] as const).map((selectedLayout) => (
                                            <button
                                                ...
                                                className={`p-4 rounded-lg border-2 transition-colors ${layout === selectedLayout ? "bg-gray-50" : "border-gray-200 hover:border-gray-400"}`}
                                                style={layout === selectedLayout ? {borderColor: accentColor} : {}}
                                            >
                                                <div
                                                    className="aspect-[8.5/11] bg-white rounded shadow-sm"/>

                                                <p className="text-sm mt-2">
                                                    {selectedLayout === "single" ? "Single Column" : "Two Column"}
                                                </p>
                                            </button>))}
                                    </div>
                                </div>
