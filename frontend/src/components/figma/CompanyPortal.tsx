...
//TODO resize buttons view applicants and best matches
...
export function CompanyPortal({ onBack }: CompanyPortalProps) {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      const dashboardJobs = data.jobs ?? [];
      setStats(data.stats ?? emptyStats);
      setCompanyName(data.company?.company_name || dashboardJobs[0]?.company_name || "Company Portal");

      // Direct reload fallback: this prevents posted jobs from disappearing if /company/dashboard
      // returns stats but an empty jobs array because of a stale backend route or partial deployment.
      if (dashboardJobs.length > 0) {
        setJobs(dashboardJobs);
      } else {
        const directJobs = await api.get<CompanyJob[]>("/company/jobs/my-jobs");
        setJobs(directJobs ?? []);
      }
    } catch (err) {
      try {
        ...
        setStats({
          ...emptyStats,
          active_jobs: (directJobs ?? []).filter((job) => job.is_active).length,
        });
        setCompanyName(directJobs?.[0]?.company_name || "Company Portal");
      } catch {
