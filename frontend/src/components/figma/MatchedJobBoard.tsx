...
import {
  ArrowLeft, Briefcase, MapPin, DollarSign, Clock, Star,
  Building2, TrendingUp, FileUser, Loader2, X,
} from 'lucide-react';
...
function extractSkillNames(content: Record<string, unknown> | null | undefined): string[] {
  if (!content) return [];
...
function getMatchingSkills(userSkills: string[], requiredSkills: string[]): string[] {
  const userLower = new Set(userSkills.map((s) => s.toLowerCase()));
  return (requiredSkills ?? []).filter((s) => userLower.has(s.toLowerCase()));
}
...
export function MatchedJobBoard({ onBack }: MatchedJobBoardProps) {
  ...
  const runMatching = useCallback(async (resumeId: string, resume: Resume) => {
    setMatching(true);
    setError(null);
    try {
      const result = await api.post<MatchResponse>(`/jobs/match-my-resume/${resumeId}`);
      if (!result || typeof result !== 'object') {
        throw new Error('Unexpected response from server.');
      }
      const filtered = (result.matches ?? []).filter((m) => m.match_score > 0);
      setMatches(filtered);
  ...
  const handleSubmitApplication = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMatches((prev) =>
        prev.map((m) =>
          m.job_id === applyJob.job_posting.id ? { ...m, status: 'applied' } : m
        )
      );
  ...
  const highMatchCount = matches.filter((m) => Math.round(m.match_score * 100) >= 90).length;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="flex items-start justify-between gap-6">
            ...
            <button
              ...
              className="bg-[#088395] border-2 border-white rounded-xl shadow-sm p-6 flex items-center gap-4 hover:shadow-md transition-all duration-200 cursor-pointer min-w-[200px] self-end mb-[-12px]"
            >
              <div className="w-12 h-12 bg-[#088395]/10 rounded-lg flex items-center justify-center">
                <FileUser size={27} className="text-white" />
              </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {resumes.length > 1 && (
          <div className="mb-6 bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <label className="block text-sm font-semibold mb-2">Match jobs against:</label>
            <select
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-foreground/70 text-sm">Good Match (70-89%)</p>
                <p className="text-2xl font-bold">{goodMatchCount}</p>
              </div>
            </div>
          </div>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-16 text-foreground/60">
            ...
            {loadingResumes ? 'Loading your resumes…' : 'Analyzing your resume against job listings…'}
          </div>
        ) : error ? (
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
            ...
            <p className="text-foreground/70 mb-4">Create a resume first to see matched jobs.</p>
            <button
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-foreground/60">
            No job matches found yet. Check back when new jobs are posted.
          </div>
        ) : (
          <div>
            <h2 className="text-2xl font-bold mb-6">Recommended Jobs ({matches.length})</h2>
                  ...
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2 flex-wrap">
                          ...
                          <div className={`flex items-center gap-1 px-3 py-1 rounded-full ${getMatchColor(match.match_score)}`}>
                            <Star size={14} className="fill-current" />
                            <span className="text-sm font-semibold">{scorePct}% Match</span>
                          </div>
                          {isApplied && (
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-foreground/70 mb-3">
                          ...
                          <div className="flex items-center gap-1">
                            <Clock size={14} /> Posted {formatPostedDate(job.created_at)}
                          </div>
                        </div>

                        {job.description && (
                          <p className="text-foreground/80 text-sm mb-4 line-clamp-2">{job.description}</p>
                        )}

                        {matchingSkills.length > 0 && (
                          <div className="mb-4">
                            <h4 className="text-sm font-semibold text-foreground/70 mb-2">Your Matching Skills:</h4>
                        ...
                        {isExpanded && (job.required_skills ?? []).length > 0 && (
                          <div className="mb-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                            <h4 className="text-sm font-semibold mb-2">All Required Skills:</h4>
                            <div className="flex flex-wrap gap-2">
                              {job.required_skills.map((skill, i) => {
                                const matched = userSkills.map((s) => s.toLowerCase()).includes(skill.toLowerCase());
                                return (
                                  <span
                                    key={i}
                    ...
                    <div className="flex gap-3">
                      {isApplied ? (
                        ...
                        >
                          <Briefcase size={16} /> Already Applied
                        </button>
                      ) : (
                        <button
      ...
      {applyJob && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-8 relative max-h-[90vh] overflow-y-auto">
            ...
            <h2 className="text-2xl font-bold mb-2">Apply for {applyJob.job_posting.job_title}</h2>
            <p className="text-foreground/70 mb-6">at {applyJob.job_posting.company_name ?? 'Unknown Company'}</p>
...
//TODO change the percentage basis for job matching cause currently
// it's somewhat low