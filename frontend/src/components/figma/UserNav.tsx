    ...
    <nav className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-md z-50 border-b border-border">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* CENTER LINKS (ONLY FOR INDIVIDUAL USERS). Order: Create CV (free) first. */}
          <div className="hidden md:flex items-center gap-7 absolute left-1/2 -translate-x-1/2">
            {!isCompany && (
              <>
                <button
                  onClick={() => router.push("/templates/showcase")}
                  className={`flex items-center gap-1.5 font-semibold ${getLinkClass(currentPage === "templates")}`}
                >
                  <FileText size={16} />
                  Create CV
                  <span className="ml-1 px-1.5 py-0.5 bg-green-100 text-green-700 rounded text-[10px] font-bold">FREE</span>
                </button>
                <button onClick={() => router.push("/create/cover-letter")} className={getLinkClass(false)}>
                  Cover Letter
                </button>
