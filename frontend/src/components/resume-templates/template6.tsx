...
const Template6: React.FC<Props> = ({ resumeData }) => {
  ...
  const sectionClass = "mt-5 border-b border-gray-300 pb-4 last:border-b-0";
  const headingClass = "text-[16px] font-bold uppercase mb-3";
  const titleClass = "text-[13px] font-bold";
  const smallTextClass = "text-[11px] mt-1";
  const descriptionClass = "text-[10px] mt-2 leading-4";
  const metaClass = "text-right text-[10px] text-gray-700 shrink-0";
              ...
              <div key={index}>
                <div className="flex justify-between items-start gap-4">
                  <div className="min-w-0">
                    {hasText(school) && (
                      <h3 className={titleClass}>{school}</h3>
                    )}
          ...
          {filledLinks.map((link: any, index: number) => (
            <div key={index}>
              {hasText(link.platform) && (
                <h3 className="text-[12px] font-bold">{link.platform}</h3>
              )}
  ...
  const renderSkills = () => {
    ...
    return (
      <section key="skills" className="mt-5 border-b border-gray-300 pb-4 last:border-b-0">
          ...
          {filledSkills.map((skill: any, index: number) => {
            ...
            return (
              <div key={index}>
                ...
                {hasText(level) && (
                  <p className={smallTextClass}>{level}</p>
                )}
            ...
            return (
              <div key={index}>
                {hasText(languageName) && (
                  <h3 className="text-[12px] font-bold">{languageName}</h3>
                )}
