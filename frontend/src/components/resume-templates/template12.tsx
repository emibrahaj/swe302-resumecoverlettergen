...
const Template12: React.FC<Props> = ({ resumeData, styleConfig }) => {
  const {
    personalInfo = {},
    summary = "",
    links = [],
...
const allProfiles = onlineSource
  ...
  const filledInterests = asArray(interests).filter((interest: any) => {
    ...
    return text(interest?.name, interest?.title, interest?.description);
  });
  ...
  const ProgressBar = ({ progress }: { progress: number }) => (
    <div
      className="w-full h-[6px] mt-1 border"
      ...
      <div className="text-center px-4 pt-5 pb-3">
        {photoUrl && (
          <img
            src={photoUrl}
            alt="profile"
            className="w-[122px] h-[132px] object-cover mx-auto"
          />
        )}

        {fullName && (
          <h1 className={`text-[20px] font-bold leading-tight mt-3 ${wrapClass}`}>
            {fullName}
          </h1>
        )}
  ...
  const renderContact = () => {
    ...
    return (
      <div className="mx-4 mb-4 border border-[#8da4a6] p-3 text-[9.5px] leading-4 space-y-1">
            ...
            return (
              <div key={index} className={wrapClass}>
                <div className="flex justify-between gap-4 min-w-0">
                  ...
                  {(dates || text(edu.location) || text(edu.gpa)) && (
                    <div className="text-right text-[9.5px] leading-4 shrink-0 max-w-[190px] break-words">
                      {degree && <p>{degree}</p>}
                      {text(edu.gpa) && <p>{text(edu.gpa)}</p>}
  ...
  const renderTraining = (sectionId: string) => {
    ...
    return renderSimpleListSection(sectionId, section.title, asArray(section.items));
  };
  ...
  return (
    <ResumePage>
      <div
        className="flex h-[1123px] bg-white text-[#111] text-[10.5px] leading-[1.35] overflow-hidden"
        style={{ fontFamily }}
      >
        {/* LEFT SIDEBAR */}
        <aside
          className="w-[35%] h-full shrink-0 overflow-hidden"
        ...
        {/* MAIN CONTENT */}
        <main className="w-[65%] h-full p-5 overflow-hidden min-w-0">
